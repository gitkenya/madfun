...
export default async function Page() {
  const currentHotel = hotels.find(
    (obj: any) => obj.slug === "nyumbu-luxury-camp"
  );
  const galleryLength = 8;
  const gallery = Array.from({ length: galleryLength }, (v, i) => ({
    id: `${i + 1}`,
    photo: `/assets/img/hotels/gallery/nyumbu/gallery-${i + 1}.jpeg`,
  }));
    ...
    <section className="w-full pt-16 sm:pt-20">
      <div className="w-full max-w-[85rem] mx-auto relative min-h-[240px] sm:min-h-[360px] sm:rounded-xl mt-0 mb-6 sm:mt-6">
        <Image
          ...
          src={`/assets/img/hotels/banners/nyumbu-luxury-camp.jpg`}
          fill
          priority
      ...
      <div className="w-full bg-slate-100">
        <OtherHotels slug={`nyumbu-luxury-camp`} />
      </div>
    </section>
  );
}
