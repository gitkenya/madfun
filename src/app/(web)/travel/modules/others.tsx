...
export default function OtherDestinations(props: any) {
  const { slug } = props;
  const currentDestination = destinations.find((obj: any) => obj.slug === slug);
  const otherDestinations: any[] = destinations
    ?.filter((obj: any) => obj.slug !== slug) // Step 1: Filter out the current destination
    .sort(
      (a: any, b: any) =>
        Math.abs(a.id - currentDestination.id) -
        Math.abs(b.id - currentDestination.id) // Step 2: Sort by proximity in ID
    )
    .slice(0, 4);
  return (
                ...
                <div className="flex flex-col sm:flex-row justify-end sm:items-end sm:justify-start gap-1 sm:gap-3 text-white text-sm">
                  ...
                  {des.from && (
                    <div className="flex flex-row items-center gap-1">
                      ...
                      <span>{parseFloat(des.from).toLocaleString()}</span>
                    </div>
                  )}
                </div>
              </div>
